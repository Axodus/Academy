...
export const rewardGateService = {
  ...
  getRewardClass(courseId: string): RewardClass {
    ...
    return course?.accessType === "paid" ? "applied" : "foundation";
  },

  getRewardTypeLabel(courseId: string) {
    return academyData.courses.find((item) => item.id === courseId)?.previewPointTier ?? "Foundation Preview";
  },
  ...
  getUnlockedAmount(courseId: string) {
    return this.getRewardGates(courseId)
      ...
      .reduce((sum, gate) => sum + gate.previewPoints, 0);
  },
